...
from src.onchain.utils.to_script_context import to_tx_out_ref, to_fraction
...
def main(
    ...
    for amount_filled in range(min(len(found_orders), max_amount), 0, -1):
        ...
        for i, (order_utxo, order_datum) in enumerate(found_orders_filtered[:1]):
            ...
            fill_order_redeemer = pycardano.Redeemer(
                orderbook.PartialMatch(
                    ...
                    ratio=to_fraction(adjusted_ratio),
                )
            )
        ...
        for i, (order_utxo, order_datum) in enumerate(
            found_orders_filtered[1:], start=1
        ):
            order_input_index = all_inputs_sorted.index(order_utxo)
            order_output_index = i
            fill_order_redeemer = pycardano.Redeemer(
